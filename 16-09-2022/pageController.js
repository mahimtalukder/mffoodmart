...
async function scrapeAll(browserInstance) {
    ...
    try {
        ...
        for (let i = 0; i < allScrapingInfo["name"].length; i++) {
            scrapedData[allScrapingInfo["name"][i]] = await pageScraper.scraper(browser, allScrapingInfo["url"][i], allScrapingInfo["name"][i]);
        }
        await browser.close();

        const today_date = new Date().toISOString().split('T')[0];

        fs.writeFile("data_of_" + today_date + ".json", JSON.stringify(scrapedData), 'utf8', function (err) {
            if (err) {
